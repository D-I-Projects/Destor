...
# Current Version
current_version = "v24.11.12-official"

# Repo url
repo_url = "https://api.github.com/repos/D-I-Projects/Destor/tags"
...
def get_java_version():
    try:
        ...
        version_match = re.search(
            r"\"(\d+\.\d+\.\d+_\d+|\d+\.\d+|\d+\.\d+\.\d+)\"", version_line
        )
        if version_match:
...
def download_icon(icon_url, file_name):
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"The icon file has been successfully downloaded and saved as {file_path}."
            )
        else:
            logger.error(
                f"Failed to download the icon file. Status code: {response.status_code}"
            )
...
if __name__ == "__main__":
    icon_url = "https://raw.githubusercontent.com/wfxey/wfxey/main/De.png"
    file_name = "icon.png"
...
class Config:
    ...
    def get_data(self, section, key, default=None):
        ...
        try:
            ...
            logger.error(
                "Section '%s' or key '%s' not found in the configuration.", section, key
            )
            return default
...
archivo_black_font_path = os.path.join(
    "SC_FILES", "font", "archivo_black", "ArchivoBlack-Regular.ttf"
)
...
server_builder_lbl = ctk.CTkLabel(
    sidebar_fm,
    text="DESTOR",
    font=("ArchivoBlack-Regular", 24, "bold"),
    text_color="white",
    bg_color="black",
)
...
home_btn = btk.BTkButton(
    ...
    width=180,
    height=30,
    command=lambda: switch(home_page),
)
home_btn.pack(fill=tk.X, pady=5)

software_btn = btk.BTkButton(
    options_fm,
    text="Server",
    bg_color="#1E90FF",
    fg_color="white",
    ...
    width=180,
    height=30,
    command=lambda: switch(server_page),
)
software_btn.pack(fill=tk.X, pady=5)

settings_btn = btk.BTkButton(
    options_fm,
    text="Settings",
    bg_color="#1E90FF",
    fg_color="white",
    ...
    width=180,
    height=30,
    command=lambda: switch(settings_page),
)
settings_btn.pack(fill=tk.X, pady=5)

server_builder_lbl = tk.Label(
    sidebar_fm,
    text=f"Destor {current_version}",
    font=("Open Sans", 8),
    fg="white",
    bg="black",
)
...
def home_page():
    ...
    # Elements for News
    label1 = ctk.CTkLabel(
        text="News", font=("Open Sans", 25), master=tabview.tab("News")
    )
    label1.pack(padx=20, pady=10)
    label = ctk.CTkLabel(
        text="Coming soon!", font=("Open Sans", 17), master=tabview.tab("News")
    )
    label.pack()

    # Elements for Changelog
    label = ctk.CTkLabel(
        text="Changelog", font=("Open Sans", 25), master=tabview.tab("Changelog")
    )
    label.pack(padx=20, pady=10)
    label = ctk.CTkLabel(
        text="- New design (Nav Buttons)", font=("Open Sans", 13), master=tabview.tab("Changelog")
    )
    label.pack()
    label = ctk.CTkLabel(
        text="- Added more Server Versions",
        font=("Open Sans", 13),
        master=tabview.tab("Changelog"),
    )
    label.pack()
    label = ctk.CTkLabel(
        text="- Included Python file", font=("Open Sans", 13), master=tabview.tab("Changelog")
    )
    label.pack()
    label = ctk.CTkLabel(
        text="- Better logging", font=("Open Sans", 13), master=tabview.tab("Changelog")
    )
    label.pack()
    label = ctk.CTkLabel(
        text="Contact us on Discord for more Informations!",
        font=("Open Sans", 15),
        master=tabview.tab("Changelog"),
    )
    label.pack()
    ...
    # Elements for About
    label = ctk.CTkLabel(
        text="About", font=("Open Sans", 25), master=tabview.tab("About")
    )
    label.pack(padx=20, pady=10)
    ...
    label = ctk.CTkLabel(
        text=f"Destor {current_version}",
        font=("Open Sans", 13),
        master=tabview.tab("About"),
    )
    label.pack()
    label = ctk.CTkLabel(
        text="@wfxey, @ivole32", font=("Open Sans", 13), master=tabview.tab("About")
    )
    label.pack()
    label = ctk.CTkLabel(
        text="https://discord.gg/A5reDWGkhV",
        font=("Open Sans", 13),
        master=tabview.tab("About"),
    )
    label.pack()
    label = ctk.CTkLabel(
        text="Licenses are inside the licenses files in the LICENSES folder that is inncluded in the GitHub repository.",
        font=("Open Sans", 13),
        master=tabview.tab("About"),
    )
    label.pack()
    label = ctk.CTkLabel(
        text="Copyright (c) 2024 D&I Projects",
        font=("Open Sans", 13),
        master=tabview.tab("About"),
    )
    label.pack()
...
def server_page():
    ...
    create_server_page_btn = ctk.CTkButton(
        server_frame,
        text="Create Server",
        font=("Open Sans", 15),
        command=lambda: switch(create_server_page),
    )
    create_server_page_btn.pack(padx=20, pady=15)

    server_label = ctk.CTkLabel(
        server_frame, text="Your Servers", font=("Open Sans", 25)
    )
    server_label.pack(pady=15)
    ...
    for i in range(1, 6):
        server_config, success = read_specific_value_from_config(
    ...
    if server_buttons:
        confirm_button = ctk.CTkButton(
            server_frame, text="Confirm", command=lambda: switch(dashboard_page)
        )
...
def delete_server():
    selected_option = (
        radio_var.get()
    )  # Stellen Sie sicher, dass radio_var definiert ist

    if selected_option == 0:
        switch(server_page)
        return

    after_delete_messagebox = messagebox.askyesno(
        "Destor",
        "If you delete the server files there is no way to recover them! Are you sure you want to continue?",
    )
    ...
    if os.path.isfile(datei_pfad):
        ...
        switch(server_page)
        print(
            f"The file '{datei_pfad}' that is located in '{directory}' was deleted successfully."
        )
    else:
        print(
            f"The file '{datei_pfad}' that is located in '{directory}' does not exist."
        )
...
class Config_Server:
    ...
    def start_download(self, jar_url, server_dir, progress_var, progress_label, root):
        ...
        threading.Thread(
            target=self.download_jar,
            args=(jar_url, server_dir, progress_var, progress_label, root),
        ).start()
    ...
    def show_progress_window(self, jar_url, server_dir):
        ...
        label = ctk.CTkLabel(
            root,
            text="By downloading, you accept Minecraft's EULA.",
            font=("Open Sans", 12),
        )
...
def create_server_page():
    ...
    def confirm_create_server():
        ...
        if not success:
            messagebox.showwarning(
                "Limit reached", "You can only own 5 servers at the same time!"
            )

    def validate_inputs():
        if (
            entry.get()
            and selected_option.get() != "Select Version"
            and software_option.get() != "Select Software"
        ):
    ...
    def option_changed(choice):
        ...
        if choice != "Select Version":
            ...
            if selected_option.get() == "1.21.3":
                software_menu.configure(values=["Vanilla", "Paper"])
            elif selected_option.get() == "1.21.2":
                software_menu.configure(values=["Vanilla"])
            elif selected_option.get() == "1.21.1":
                software_menu.configure(values=["Vanilla", "Paper"])
            elif selected_option.get() == "1.3.1":
                software_menu.configure(values=["Bukkit", "Vanilla"])
            elif selected_option.get() == "1.2.5":
                software_menu.configure(values=["Bukkit", "Vanilla"])
            else:
    ...
    settings_label = ctk.CTkLabel(
        create_server_frame, text="Create Server", font=("Open Sans", 25)
    )
    settings_label.pack(pady=15)

    name_label = ctk.CTkLabel(
        create_server_frame, text="Server Name", font=("Open Sans", 13)
    )
    name_label.pack()
    ...
    software_label = ctk.CTkLabel(
        create_server_frame, text="Server Version", font=("Open Sans", 13)
    )
    ...
    software_label = ctk.CTkLabel(
        create_server_frame, text="Server Software", font=("Open Sans", 13)
    )
    software_label.pack(pady=(20, 0))

    software_option = ctk.StringVar(value="Select Software")
    software_menu = ctk.CTkOptionMenu(
        create_server_frame,
        values=[],
        variable=software_option,
        command=lambda choice: validate_inputs(),
    )
...
def remove_plugin():
    ...
    file_path = filedialog.askopenfilename(
        initialdir=initial_dir, filetypes=[("Jar files", "*.jar")]
    )
    if not file_path:
        return
    try:
...
def plugins_page():
    ...
    global listbox
    plugins_folder_path = os.path.join(
        directory, f"Server_{selected_option}", "plugins"
    )
    ...
    create_button2 = ctk.CTkButton(
        button_frame, text="Remove plugin", command=remove_plugin
    )
...
def dashboard_page():
    ...
    global console_text
    console_text = ctk.CTkTextbox(dashboard_frame, wrap=tk.WORD, width=700, height=450)
    ...
    plugins_button = ctk.CTkButton(
        ...
        text="Plugins",
        font=("Open Sans", 14),
        command=lambda: switch(plugins_page),
    )
...
def start_server():
    ...
    try:
        ...
        server_process = subprocess.Popen(
            [
                "java",
                f"-Xmx{min_heap_size}G",
                f"-Xms{max_heap_size}G",
                "-jar",
                str(jar_path),
                "nogui",
            ],
        ...
        threading.Thread(
            target=read_output, args=(server_process.stdout, "info")
        ).start()
        threading.Thread(
            target=read_output, args=(server_process.stderr, "error")
        ).start()
...
def read_output(pipe, level):
    """Reads output from the server process and updates the console."""
    for line in iter(pipe.readline, b""):
        line = line.decode("utf-8").strip()
        update_console(line, level)
    pipe.close()
...
def update_console(text, level):
    ...
    if console_text:
        ...
        console_text.insert(
            tk.END, text + "\n", ("info",)
        ) if level == "info" else console_text.insert(tk.END, text + "\n", ("error",))
...
def handle_keypress(event):
    ...
    if console_text:
        if event.keysym not in ["BackSpace", "Delete"]:
            ...
            console_text.tag_config(
                "input", foreground="green"
            )  # Custom tag for input text
...
def show_game_rules():
    ...
    back_button = ctk.CTkButton(
        ...
        fg_color="red3",
        hover_color="red4",
        command=lambda: switch(dashboard_page),
    )
...
def get_jar_url(version, software):
    jar_urls = {
        "1.21.3": {
            ...
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.21.3/builds/27/downloads/paper-1.21.3-27.jar",
        },
        "1.21.2": {
            "Vanilla": "https://piston-data.mojang.com/v1/objects/7bf95409b0d9b5388bfea3704ec92012d273c14c/server.jar"
        },
        "1.21.1": {
            "Vanilla": "https://piston-data.mojang.com/v1/objects/59353fb40c36d304f2035d51e7d6e6baa98dc05c/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.21.1/builds/81/downloads/paper-1.21.1-81.jar",
        },
        "1.21": {
            ...
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.21/builds/130/downloads/paper-1.21-130.jar",
        },
        "1.20.6": {
        ...
        "1.12.1": {
            "Bukkit": "https://getbukkit.org/get/rqnfCJX7MN2S2whTbzdmGcHIUSC0MC2E",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.12.1.jar",
            "Vanilla": "https://piston-data.mojang.com/v1/objects/561c7b2d54bae80cc06b05d950633a9ac95da816/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.12.1/builds/1204/downloads/paper-1.12.1-1204.jar",
        },
        "1.12": {
            "Bukkit": "https://getbukkit.org/get/bf8d6ecd2fb20108e039012b763fe36e",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.12.jar",
            "Vanilla": "https://piston-data.mojang.com/v1/objects/8494e844e911ea0d63878f64da9dcc21f53a3463/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.12/builds/1169/downloads/paper-1.12-1169.jar",
        },
        "1.11.2": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.11.2.jar",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.11.2.jar",
            "Vanilla": "https://piston-data.mojang.com/v1/objects/f00c294a1576e03fddcac777c3cf4c7d404c4ba4/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.11.2/builds/1106/downloads/paper-1.11.2-1106.jar",
        },
        "1.10.2": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.10.2-R0.1-SNAPSHOT-latest.jar",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.10.2-R0.1-SNAPSHOT-latest.jar",
            "Vanilla": "https://piston-data.mojang.com/v1/objects/3d501b23df53c548254f5e3f66492d178a48db63/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.10.2/builds/918/downloads/paper-1.10.2-918.jar",
        },
        "1.9.4": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.9.4-R0.1-SNAPSHOT-latest.jar",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.9.4-R0.1-SNAPSHOT-latest.jar",
            "Vanilla": "https://piston-data.mojang.com/v1/objects/edbb7b1758af33d365bf835eb9d13de005b1e274/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.9.4/builds/775/downloads/paper-1.9.4-775.jar",
        },
        "1.8.8": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.8.8-R0.1-SNAPSHOT-latest.jar",
            "Spigot": "https://cdn.getbukkit.org/spigot/spigot-1.8.8-R0.1-SNAPSHOT-latest.jar",
            "Vanilla": "https://launcher.mojang.com/v1/objects/5fafba3f58c40dc51b5c3ca72a98f62dfdae1db7/server.jar",
            "Paper": "https://api.papermc.io/v2/projects/paper/versions/1.8.8/builds/445/downloads/paper-1.8.8-445.jar",
        },
        "1.3.1": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.3.1-R2.1-SNAPSHOT.jar",
            "Vanilla": "https://launcher.mojang.com/v1/objects/82563ce498bfc1fc8a2cb5bf236f7da86a390646/server.jar",
        },
        "1.2.5": {
            "Bukkit": "https://cdn.getbukkit.org/craftbukkit/craftbukkit-1.2.5-R5.1-SNAPSHOT.jar",
            "Vanilla": "https://launcher.mojang.com/v1/objects/d8321edc9470e56b8ad5c67bbd16beba25843336/server.jar",
        },
    }
...
def settings_page():
    ...
    settings_label = ctk.CTkLabel(
        settings_frame, text="General", font=("Open Sans", 25)
    )
    settings_label.pack(pady=15)
    ...
    def create_function_frame(
        ...
        description = ctk.CTkLabel(
            function_frame, text=description_text, font=("Open Sans", 15)
        )
    ...
    settings_label = ctk.CTkLabel(
        settings_frame, text="Developer", font=("Open Sans", 25)
    )
    settings_label.pack(pady=15)
    ...
    if response.status_code == 200:
        ...
        if tags:
            ...
            if version.parse(latest_tag) > version.parse(current_version):
                logger.info(
                    f"New version available : {latest_tag} / https://github.com/wfxey/Destor/releases/tag/{current_version}."
                )
                messagebox.showinfo(
                    "Destor",
                    f"New version available: {latest_tag}. Current version : {current_version}.",
                )
            else:
                logger.info("Destor is up to date.")
                messagebox.showinfo(
                    "Destor",
                    f"Destor is up to date. Current version : {current_version}.",
                )
        else:
