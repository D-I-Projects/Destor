...
#Current Version
current_version = "v24.6.21-alpha"
...
x1 = (screen_width1 - window_width) // 2
y1 = (screen_height1 - window_height) // 2
app.geometry(f"1100x600+{x1}+{y1}")
...
def start_server():
    ...
    jar_path = os.path.join(directory, f"Server_{selected_option}", "server.jar")
    
    process = subprocess.Popen([f"java -Xmx{ram_value / 2}G -Xms{ram_value}G -jar {jar_path} nogui"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def dashboard_page():
    ...
    start_button = ctk.CTkButton(button_frame, text="Start Server", font=("Open Sans", 14), command=start_server_thread)
    ...
    back_button = ctk.CTkButton(button_frame, text="Back", font=("Open Sans", 14), fg_color="red3", hover_color="red4", command=lambda:switch(server_page))
    back_button.pack(side=tk.RIGHT, padx=10, pady=10)
